...
- `--session` existing session token
- `--verify` verify TLS certificates

If you provide `hostname` without a port, the CLI assumes port `9445` and always uses `https://.../api/`.

## When to use `--session`

`--session` is primarily meant for TestBench 4.x, where the API issues a session token.
...
- `sessionToken`: a session token (usually TestBench 4.x)
